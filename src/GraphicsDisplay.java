...
public class GraphicsDisplay extends JPanel {
    ...
    public GraphicsDisplay() {
        ...
        this.graphicsStroke = new BasicStroke(5.0f, BasicStroke.CAP_BUTT, BasicStroke.JOIN_ROUND, 10.0f, new float[] {20, 5, 10, 5}, 0.0f);
        ...
        this.markerStroke = new BasicStroke(3.0F, 0, 0, 10.0F, (float[])null, 0.0F);
        ...
        this.smallfont = new Font("SansSerif", Font.ITALIC, 12);
    ...
    public void zoomToRegion(double x1, double y1, double x2, double y2) {
        this.viewport[0][0] = x1; //минимальный x
        this.viewport[0][1] = y1; //максимальный y
        this.viewport[1][0] = x2; //максимальный x
        this.viewport[1][1] = y2; //минимальный y
        this.repaint();
    }
    ...
    private void paintMarkers(Graphics2D canvas) {
        ...
        java.awt.geom.Line2D.Double lastline=null;
        java.awt.geom.Line2D.Double lastline1=null;
        int i = -1;
        Iterator var5 = this.graphicsData.iterator();
        ...
        while(var5.hasNext()) {
            ...
            if (point[0] >= this.viewport[0][0] && point[1] <= this.viewport[0][1] && point[0] <= this.viewport[1][0] && point[1] >= this.viewport[1][1]) {
                ...
                Point2D.Double corner = translateXYtoPoint(center.getX()+radius,center.getY()+radius);
                marker.setFrameFromCenter(center, corner);
                Line2D.Double line = new Line2D.Double(translateXYtoPoint(center.getX()-radius, center.getY()), translateXYtoPoint(center.getX()+radius, center.getY()));*/
                ...
                for(int j = 0; j<temp.length()-1;j++){
                    if((int)temp.charAt(j)>(int)temp.charAt(j+1)) {
                        highervalue=false;
                        break;
                    }
                }
                Line2D.Double line = new Line2D.Double(center.getX()-radius, center.getY(),center.getX()+radius, center.getY());
                Line2D.Double line1 = new Line2D.Double(center.getX(), center.getY()-radius, center.getX(),center.getY()+radius);
        ...
        if (lastMarker != null) {
            ...
            canvas.draw(lastline);
            canvas.draw(lastline1);
    ...
    protected void paintIntegrals(Graphics2D canvas) {
        ...
        for (int i = 0; i < graphicsData.size() - 1; i++) {
            System.out.println("X: " + graphicsData.get(i)[0] + " Y: " + graphicsData.get(i)[1] + " i: " + i);
            if ((graphicsData.get(i)[1] < 0 == graphicsData.get(i+1)[1] >= 0) || (graphicsData.get(i)[1] == 0)) {
                if (domens != 0) {
                    domens += 1;
        ...
        if(domens>=1) {
            ...
            for (int i = 0; i < 2 * domens.intValue(); i++) {
                ...
                xcoordinates.add(-graphicsData.get(indexses.get(i))[1] / (graphicsData.get(indexses.get(i) + 1)[1] - graphicsData.get(indexses.get(i))[1]) * (graphicsData.get(indexses.get(i) + 1)[0] - graphicsData.get(indexses.get(i))[0]) + graphicsData.get(indexses.get(i))[0]);
            ...
            for (int i = 0; i < graphicsData.size(); i++) {
                ...
                if (graphicsData.get(i)[0] >= xcoordinates.get(k) && graphicsData.get(i)[0] < xcoordinates.get(k + 1)) {
// Преобразовать значения (x,y) в точку на экране point
                    if (maxy < graphicsData.get(i)[1]) {
                        maxy = graphicsData.get(i)[1];
                    }
                    if (miny > graphicsData.get(i)[1]) {
                        miny = graphicsData.get(i)[1];
                    }
                    if (graphicsData.get(i - 1)[0] <= xcoordinates.get(k)) {
// Первая итерация цикла - установить начало пути в точку point
                        integral[k / 2] += Math.abs((graphicsData.get(i)[0] - xcoordinates.get(k)) * graphicsData.get(i)[1] / 2);
                        canvas.setColor(Color.red);
                        Point2D.Double point = translateXYtoPoint(xcoordinates.get(k), 0);
                    ...
                    if (graphicsData.get(i + 1)[0] >= xcoordinates.get(k + 1)) {
// Не первая итерация цикла - вести линию в точку point
                        Point2D.Double point = translateXYtoPoint(graphicsData.get(i)[0], graphicsData.get(i)[1]);
                        path.lineTo(point.getX(), point.getY());
                        point = translateXYtoPoint(xcoordinates.get(k + 1), 0);
                        path.lineTo(point.getX(), point.getY());
                        integral[k / 2] += Math.abs(graphicsData.get(i)[1] / 2 * (xcoordinates.get(k + 1) - graphicsData.get(i)[0]));
                        path.closePath();
                        System.out.println("The line was closed , x = " + point.getX() + " on the iteration i = " + i);
                        //canvas.setPaint(new Color((int)(255*(k/2)/(xcoordinates.size()/2)), (int)(255*(k/2)/(xcoordinates.size()/2)), (int)(255*(k/2)/(xcoordinates.size()/2))));
                        canvas.setPaint(new Color((int) (255 * Math.random()), (int) (255 * Math.random()), (int) (255 * Math.random())));
                    ...
                    if (!(graphicsData.get(i + 1)[0] >= xcoordinates.get(k + 1)) && !(graphicsData.get(i - 1)[0] <= xcoordinates.get(k))) {
                        integral[k / 2] += Math.abs((graphicsData.get(i)[0] - graphicsData.get(i - 1)[0]) * (graphicsData.get(i)[1] + graphicsData.get(i - 1)[1]) / 2);
                        Point2D.Double point = translateXYtoPoint(graphicsData.get(i)[0], graphicsData.get(i)[1]);
                        path.lineTo(point.getX(), point.getY());
                    }
            ...
            for (
                ...
                canvas.drawString(String.format("%.3f", integral[i]), (float) (translateXYtoPoint(xcoordinates.get(2 * i) + (xcoordinates.get(2 * i + 1) - xcoordinates.get(2 * i)) / 2 - bounds.getX(), averagey[i] / 2).getX()), (float) translateXYtoPoint(xcoordinates.get(2 * i) + (xcoordinates.get(2 * i + 1) - xcoordinates.get(2 * i)) / 2, averagey[i] / 2).getY());
            }
        }
    }
    ...
    protected int findSelectedPoint(int x, int y) {
        if (this.graphicsData == null) {
            ...
            for(Iterator var5 = this.graphicsData.iterator(); var5.hasNext(); ++pos) {
                ...
                double distance = Math.sqrt((screenPoint.getX() - (double)x) * (screenPoint.getX() - (double)x) + (screenPoint.getY() - (double)y) * (screenPoint.getY() - (double)y));
                if (distance < 10.0D) {
                    return pos;
                }
            }
    ...
    public class MouseMotionHandler implements MouseMotionListener {
        ...
        public void mouseDragged(MouseEvent ev) {
            if (GraphicsDisplay.this.changeMode) {
                ...
                if (newX > GraphicsDisplay.this.viewport[1][0]) {
                    newX = GraphicsDisplay.this.viewport[1][0];
                }

                if (newX < GraphicsDisplay.this.viewport[0][0]) {
                    newX = GraphicsDisplay.this.viewport[0][0];
                }
                ((Double[])GraphicsDisplay.this.graphicsData.get(GraphicsDisplay.this.selectedMarker))[1] = newY;
                ((Double[])GraphicsDisplay.this.graphicsData.get(GraphicsDisplay.this.selectedMarker))[0] = newX;
                GraphicsDisplay.this.repaint();
            } else {
